...
server.post('/accendi-luci', function (req, res, next) {
    ...
    mqttClient.publish('iot2025test/test.mosquitto.org/lightstate', 'on', function (err) {
        if (err) {
...
function sendCommand() {
    ...
    rl.question('Inserisci il comando (on per accendere, off per spegnere): ', function (command) {
        ...
        if (command === 'on' || command === 'off') {
            mqttClient.publish('iot2025test/test.mosquitto.org/lightstate', command, function (err) {
                if (err) {
